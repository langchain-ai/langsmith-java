...
Validated 2026-09-22 against the open Dependabot alerts for `langsmith-java-staging`.

## Status

| Item | Scope | Status |
|------|-------|--------|
| Jackson 2.13.4/2.14.0/2.18.6 -> 2.22.1 | Published SDK + examples + tests | Done |
| Spring Boot 2.7.18 -> 4.1.1 | Example module only, non-published | Done |
...
| WireMock JRE8 2.35.2 -> org.wiremock:wiremock 4.0.0-beta.38 | Test scope only | Done |
| Jetty test constraints -> 12.1.10 | Test scope only | Done |
| Bouncy Castle test constraints -> 1.86 | Test scope only | Done |
| Log4j Core test constraint -> 2.26.1 | Test scope only | Done |
| OpenNLP test constraint -> 2.5.9 | Test scope only | Done |
| Handlebars test constraint -> 4.5.2 | Test scope only | Done |
| Plexus Utils test constraint -> 4.1.0 | Test scope only | Done |
...
Jetty 12. Keeping WireMock 3 would still leave Jetty 11 in at least one advisory range. WireMock
4.0.0-beta.38 still declares Bouncy Castle 1.84, so both WireMock dependencies exclude that group and
the tests add Bouncy Castle 1.86 directly.
