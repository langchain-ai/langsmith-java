...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { LangsmithApiInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw LangsmithApiInvalidDataException("`$name` is not set")
            is JsonNull -> throw LangsmithApiInvalidDataException("`$name` is null")
            else -> throw LangsmithApiInvalidDataException("`$name` is invalid, received $this")
        }
