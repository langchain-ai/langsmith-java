...
private constructor(
    ...
    companion object {
        ...
        /** Returns a mutable builder for constructing an instance of [BodyParamsForRunSchema]. */
        @JvmStatic fun builder() = Builder()
    }

    /** A builder for [BodyParamsForRunSchema]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(bodyParamsForRunSchema: BodyParamsForRunSchema) = apply {
            id = bodyParamsForRunSchema.id.map { it.toMutableList() }
            cursor = bodyParamsForRunSchema.cursor
            dataSourceType = bodyParamsForRunSchema.dataSourceType
            endTime = bodyParamsForRunSchema.endTime
            error = bodyParamsForRunSchema.error
            executionOrder = bodyParamsForRunSchema.executionOrder
            filter = bodyParamsForRunSchema.filter
            isRoot = bodyParamsForRunSchema.isRoot
            limit = bodyParamsForRunSchema.limit
            order = bodyParamsForRunSchema.order
            parentRun = bodyParamsForRunSchema.parentRun
            query = bodyParamsForRunSchema.query
            referenceExample = bodyParamsForRunSchema.referenceExample.map { it.toMutableList() }
            runType = bodyParamsForRunSchema.runType
            searchFilter = bodyParamsForRunSchema.searchFilter
            select = bodyParamsForRunSchema.select.map { it.toMutableList() }
            session = bodyParamsForRunSchema.session.map { it.toMutableList() }
            skipPagination = bodyParamsForRunSchema.skipPagination
            skipPrevCursor = bodyParamsForRunSchema.skipPrevCursor
            startTime = bodyParamsForRunSchema.startTime
            trace = bodyParamsForRunSchema.trace
            traceFilter = bodyParamsForRunSchema.traceFilter
            treeFilter = bodyParamsForRunSchema.treeFilter
            useExperimentalSearch = bodyParamsForRunSchema.useExperimentalSearch
            additionalProperties = bodyParamsForRunSchema.additionalProperties.toMutableMap()
        }
        ...
        fun skipPrevCursor(skipPrevCursor: Boolean) = skipPrevCursor(JsonField.of(skipPrevCursor))
        ...
        /**
         * Returns an immutable instance of [BodyParamsForRunSchema].
         *
         * Further updates to this [Builder] will not mutate the returned instance.
         */
        fun build(): BodyParamsForRunSchema =
    ...
    fun validate(): BodyParamsForRunSchema = apply {
        if (validated) {
            return@apply
        }
    ...
    class Order @JsonCreator private constructor(private val value: JsonField<String>) : Enum {
        ...
        fun known(): Known =
            when (this) {
                ASC -> Known.ASC
                DESC -> Known.DESC
                else -> throw LangsmithApiInvalidDataException("Unknown Order: $value")
            }
    ...
    class Select @JsonCreator private constructor(private val value: JsonField<String>) : Enum {
        ...
        fun known(): Known =
            when (this) {
                ...
                else -> throw LangsmithApiInvalidDataException("Unknown Select: $value")
            }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BodyParamsForRunSchema &&
            id == other.id &&
            cursor == other.cursor &&
