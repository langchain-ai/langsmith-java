// File generated from our OpenAPI spec by Stainless.

package com.langchain.smith.models.feedback

import com.langchain.smith.core.http.QueryParams
import java.time.OffsetDateTime
import org.assertj.core.api.Assertions.assertThat
import org.junit.jupiter.api.Test

internal class FeedbackListParamsTest {

    @Test
    fun create() {
        FeedbackListParams.builder()
            .comparativeExperimentId("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
            .hasComment(true)
            .hasScore(true)
            .includeUserNames(true)
            .addKey("string")
            .addKey("string")
            .level(FeedbackLevel.RUN)
            .limit(1L)
            .maxCreatedAt(OffsetDateTime.parse("2019-12-27T18:11:19.117Z"))
            .minCreatedAt(OffsetDateTime.parse("2019-12-27T18:11:19.117Z"))
            .offset(0L)
            .runOfStrings(listOf("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"))
            .sessionOfStrings(listOf("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"))
            .addSource(SourceType.API)
            .addSource(SourceType.MODEL)
            .addUser("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
            .addUser("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
            .build()
    }

    @Test
    fun queryParams() {
        val params =
            FeedbackListParams.builder()
                .comparativeExperimentId("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
                .hasComment(true)
                .hasScore(true)
                .includeUserNames(true)
                .addKey("string")
                .addKey("string")
                .level(FeedbackLevel.RUN)
                .limit(1L)
                .maxCreatedAt(OffsetDateTime.parse("2019-12-27T18:11:19.117Z"))
                .minCreatedAt(OffsetDateTime.parse("2019-12-27T18:11:19.117Z"))
                .offset(0L)
                .runOfStrings(listOf("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"))
                .sessionOfStrings(listOf("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"))
                .addSource(SourceType.API)
                .addSource(SourceType.MODEL)
                .addUser("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
                .addUser("182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
                .build()

        val queryParams = params._queryParams()

        assertThat(queryParams)
            .isEqualTo(
                QueryParams.builder()
                    .put("comparative_experiment_id", "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
                    .put("has_comment", "true")
                    .put("has_score", "true")
                    .put("include_user_names", "true")
                    .put("key", "string")
                    .put("key", "string")
                    .put("level", "run")
                    .put("limit", "1")
                    .put("max_created_at", "2019-12-27T18:11:19.117Z")
                    .put("min_created_at", "2019-12-27T18:11:19.117Z")
                    .put("offset", "0")
                    .put("run", "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
                    .put("session", "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
                    .put("source", "api")
                    .put("source", "model")
                    .put("user", "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
                    .put("user", "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
                    .build()
            )
    }

    @Test
    fun queryParamsWithoutOptionalFields() {
        val params = FeedbackListParams.builder().build()

        val queryParams = params._queryParams()

        assertThat(queryParams).isEqualTo(QueryParams.builder().build())
    }
}
