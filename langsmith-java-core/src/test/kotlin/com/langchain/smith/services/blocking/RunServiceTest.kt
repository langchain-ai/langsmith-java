...
internal class RunServiceTest {
    ...
    @Test
    fun `auto batch queue sends create and update params attachments through multipart`() {
        val capturedRequest = AtomicReference<HttpRequest>()
        val httpClient = capturingHttpClient(capturedRequest)
        val runService = autoBatchRunService(httpClient)
        val createParams =
            RunCreateParams.builder()
                .runIngest(testRun("create-id"))
                .putAttachment(
                    "create-file",
        ...
        runService.update(updateParams)
        runService.flush()
