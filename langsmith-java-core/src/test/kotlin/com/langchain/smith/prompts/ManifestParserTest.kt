...
internal class ManifestParserTest {
    ...
    private fun buildMessageTemplate(
        className: String,
        template: String,
    ): Map<String, Any> =
        mapOf(
    ...
    fun parseChatPromptTemplate() {
        val manifest = buildChatPromptManifest(
            messages = listOf(
                buildMessageTemplate("SystemMessagePromptTemplate", "You are a helpful assistant."),
    ...
    fun parseChatPromptTemplateWithAiMessage() {
        val manifest = buildChatPromptManifest(
            messages = listOf(
                buildMessageTemplate("SystemMessagePromptTemplate", "You are an assistant."),
                buildMessageTemplate("HumanMessagePromptTemplate", "Hello"),
                buildMessageTemplate("AIMessagePromptTemplate", "Hi there! How can I help?"),
                buildMessageTemplate("HumanMessagePromptTemplate", "Tell me about {topic}"),
            ),
            inputVariables = listOf("topic"),
        )
        ...
        val manifest = buildChatPromptManifest(
            messages = listOf(
                mapOf(
                    "lc" to 1,
                    "type" to "constructor",
                    "id" to listOf("langchain_core", "prompts", "chat", "SystemMessagePromptTemplate"),
    ...
    fun parseStructuredPrompt() {
        val manifest = buildStructuredPromptManifest(
            messages = listOf(
                buildMessageTemplate("SystemMessagePromptTemplate", "You tell jokes."),
                buildMessageTemplate("HumanMessagePromptTemplate", "Tell me a joke about {topic}"),
            ),
            inputVariables = listOf("topic"),
            schema = sampleSchema,
        )
    ...
    fun parseStructuredPromptDetectedFromKwargs() {
        ...
        val manifest = mapOf(
            ...
            "kwargs" to mapOf(
                "messages" to listOf(
                    buildMessageTemplate("HumanMessagePromptTemplate", "Extract info from {text}"),
                ),
    ...
    fun parseStructuredPromptPreservesNestedSchema() {
        ...
        val nestedSchema = mapOf<String, Any>(
            "title" to "PersonInfo",
            "type" to "object",
            "properties" to mapOf(
        ...
        val manifest = buildStructuredPromptManifest(
            messages = listOf(
                buildMessageTemplate("HumanMessagePromptTemplate", "Extract: {text}"),
            ),
        ...
        @Suppress("UNCHECKED_CAST")
        val hobbies = properties["hobbies"] as Map<String, Any?>
    ...
    fun parseInvalidManifestThrows() {
        assertThatThrownBy {
            ManifestParser.parse(JsonValue.from("not an object"))
    ...
    fun parseUnrecognizedTypeThrows() {
        ...
        assertThatThrownBy {
            ManifestParser.parse(JsonValue.from(manifest))
        }.isInstanceOf(IllegalArgumentException::class.java)
