...
object OpenTelemetryConfig {
    ...
    @JvmStatic
    fun flush(): Boolean = flush(5, TimeUnit.SECONDS)
    ...
    class Builder {
        ...
        fun processorType(processorType: SpanProcessorType) = apply { this.processorType = processorType }
        ...
        fun build(): io.opentelemetry.api.OpenTelemetry {
            ...
            val spanProcessor = when (processorType) {
                SpanProcessorType.SIMPLE -> SimpleSpanProcessor.create(loggingExporter)
                SpanProcessorType.BATCH -> BatchSpanProcessor.builder(loggingExporter)
        ...
        private fun buildOtlpEndpoint(baseUrl: String?): String = OpenTelemetryConfig.buildOtlpEndpoint(baseUrl)
    }
    ...
    private class LoggingSpanExporter(private val delegate: SpanExporter) : SpanExporter {
        override fun export(spans: Collection<SpanData>): CompletableResultCode {
            if (DEBUG) {
                ...
                for (span in spans) {
                    logger.debug("  - ${span.name} (kind=${span.kind}, attributes=${span.attributes.size()})")
                }
            }
            ...
            if (DEBUG) {
                try {
                    ...
                    if (!result.isSuccess) {
                        ...
                        for (span in spans) {
                            logger.error("  - ${span.name} (traceId=${span.traceId}, spanId=${span.spanId})")
                        }
        ...
        companion object {
            private val DEBUG = java.lang.Boolean.getBoolean("langsmith.debug") ||
