...
public final class OtelSpanCreator {
     ...
     */
    public static Span createLlmSpan(Tracer tracer, String name, String system, String model, 
                                     String serviceName, String sessionId) {
     ...
     * @return a started Span with gen_ai attributes set
     */
    public static Span createToolSpan(Tracer tracer, String name, String toolName,
                                      String serviceName, String sessionId) {
        Span span = tracer.spanBuilder(name)
                .setParent(Context.current())
                .startSpan();
     ...
     * @return a started Span with gen_ai attributes set
     */
    public static Span createRetrievalSpan(Tracer tracer, String name,
                                           String serviceName, String sessionId) {
        Span span = tracer.spanBuilder(name)
                .setParent(Context.current())
                .startSpan();
     ...
     * @return a started Span with gen_ai attributes set
     */
    public static Span createChainSpan(Tracer tracer, String name,
                                       String serviceName, String sessionId) {
        Span span = tracer.spanBuilder(name)
                .setParent(Context.current())
                .startSpan();
    ...
    public static void createSpanFromRun(Tracer tracer, Run run, String projectName) {
        ...
        String traceIdHex = normalizeTraceId(run.traceId().orElseGet(() -> java.util.UUID.randomUUID().toString().replace("-", "")));
        String spanIdHex = normalizeSpanId(run.id().orElseGet(() -> java.util.UUID.randomUUID().toString().replace("-", "").substring(0, 16)));
        String parentSpanIdHex = run.parentRunId().map(OtelSpanCreator::normalizeSpanId).orElse(null);
        ...
        try {
            ...
            span.setAttribute(AttributeKey.stringKey("gen_ai.operation.name"), 
            ...
            run.sessionId().ifPresent(id -> 
                span.setAttribute(AttributeKey.stringKey("session.id"), id));
